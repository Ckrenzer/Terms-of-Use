...
A great way to find differences between the policies is by using the TF-IDF statistic.
...
We can compare words by measuring their similarity.


```{r word embedding calculation}

# Calculating unigram probabilities, extracting the probabilities
# from the data frame, and naming the vector corresponding to
# the word the probability belongs to
unigram_prob <- tos %>% 
  mutate(text = str_replace_all(text, "[,/\\\\.:;()\"\\[\\]&]|^\\s+-\\s+|\u009d", " ")) %>% 
  ngrams(text_df = ., key_column = "company", text_column = "text", n = 1) %>% 
...
#unigram_prob <- structure(.Data = unigram_prob$probability, names = unigram_prob$text)
...
word_vec <- as.matrix(pmi_matrix)


# The results of the singular value decomposition
svd_components <- singular_value_decomp(A = word_vec, singular_values_only = FALSE)
...
# removing objects we no longer need
rm(unigram_prob, skipgrams, pmi_matrix, word_vec, svd_components)
...
Now that we have the output of the word2vec algorithm, we can do word math! Even though this was not trained on nearly a large enough dataset to be trusted, we can still ask some questions.
