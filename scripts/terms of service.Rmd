...
You may find this project reminiscent of the Fed Minutes Analysis. This study differs in several ways. Namely, many of the algorithms used here are written 'by-hand' without reliance on specialized external libraries. Doing this has been incredibly helpful understanding the logic built into common frameworks for natural language processing and hopefully inspires others reading this to try re-writing algorithms they use. Further, the terms of use agreement is cross-sectional--we are not following the same organization across many years but are instead looking at different groups at one point in time. The purpose of this project is to learn more about NLP algorithms, and I think it has been a great success!
...
Other algorithms written 'by-hand' include a pairwise counting function, the TF-IDF statistic, and word2vec (word embeddings). These do not have significant enough differences in end results to provide a play-by-play.
...
We can see words like "item" are unique to Amazon, "privaci [privacy]" to DuckDuckGo, or "repositori [repository]" to GitHub. Features key to the services of these companies become clear using the TF-IDF.
...
How do we do this? With the output of the word2vec algorithm, we use word math! Even though word2vec was not trained with a nearly large enough dataset to be trusted, we can still ask questions.
